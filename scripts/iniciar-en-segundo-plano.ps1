...
$NodeLog = Join-Path $Proyecto "node.log"
$FlaskLog = Join-Path $Proyecto "flask.log"
$Npm = "C:\Program Files\nodejs\npm.cmd"
...
Start-Process -FilePath "cmd.exe" -ArgumentList "/c cd /d `"$Proyecto`" && `"$Python`" medidor\Gasolina.py > `"$FlaskLog`" 2>&1" -WindowStyle Hidden
